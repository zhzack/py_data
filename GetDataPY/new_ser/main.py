...
def main(enable_gui=False, print_serial_data=1, description=""):
    ...
    if logger.open_serial_port():
        try:
            ...
            logger.filename = os.path.join(
                data_folder, f"data_{timestamp}{description_part}.csv")
            ...
            if enable_gui:
                ...
                serial_thread = threading.Thread(
                    target=logger.read_serial_data, args=(app.update_label,))
                serial_thread.daemon = True
                serial_thread.start()

                app.start()
            else:
                logger.read_serial_data(None, queue)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Serial Data Logger with optional GUI display.")
    ...
    parser.add_argument("--desc", type=str, default="",
                        help="Description to append to the filename")
